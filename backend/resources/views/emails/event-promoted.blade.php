...
<head>
    <meta charset="UTF-8">
    <title>{{ trans('messages.emails.promoted.heading', [], $locale) }}</title>
</head>
<body style="font-family: Georgia, 'Times New Roman', serif; color: #1c1a17; background: #f7f1e3; margin: 0; padding: 24px;">
    <div style="max-width: 600px; margin: 0 auto; background: #fdfaf2; border: 1px solid #d6cdb6; padding: 32px 24px;">
        ...
        {{-- The one line that has to survive a glance at a phone lock screen. --}}
        <p style="margin: 0 0 18px; text-align: center; font-family: Arial, sans-serif; font-size: 12px; letter-spacing: 3px; text-transform: uppercase; color: #b08a3e; border: 1px solid #b08a3e; padding: 10px 12px;">
        ...
        <div style="border: 1px solid #d6cdb6; background: #f7f1e3; padding: 18px 20px; margin: 0 0 20px;">
            ...
            <p style="margin: 0 0 6px; font-size: 14px;">
                <strong>{{ trans('messages.emails.promoted.when', [], $locale) }}:</strong> {{ $eventWhen }}
            </p>

            @if ($eventLocation)
                <p style="margin: 0 0 6px; font-size: 14px;">
                    <strong>{{ trans('messages.emails.promoted.where', [], $locale) }}:</strong> {{ $eventLocation }}
                </p>
            @endif
